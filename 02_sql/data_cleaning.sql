...
SELECT
  ...
  FORMAT_TIMESTAMPT('%B', started_at) AS month,
  FORMAT_TIMESTAMPT('%b', started_at) AS day_name,
  EXTRACT(HOUR FROM started_at) AS hour,
  TIMESTAMPDIFF(ended_at, started_at, SECOND) / 60.0 AS ride_length_minutes
...
UNION ALL SELECT 'user_type', COUNT(*) FROM `project-aa35d8fc-3634-4b25-b55`.cyclistic.cyclistic_2022_data WHERE user_type IS NULL
UNION ALL
